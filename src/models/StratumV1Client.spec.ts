...
describe('StratumV1Client', () => {
    ...
    it('should save client', async () => {
        jest.spyOn(client as any, 'write').mockImplementation((data) => Promise.resolve(true));
        const insertSpy = jest.spyOn(clientService, 'insert');
        const handleMiningSubmissionSpy = jest.spyOn(client as any, 'handleMiningSubmission');
        ...
        expect(handleMiningSubmissionSpy).toHaveBeenCalled();
